...
## TD3、DDPG、SAC、A2C 的核心关键区分
|     核心特性     |              TD3              |             DDPG              |                 SAC                 |              A2C               |
| :--------------: | :---------------------------: | :---------------------------: | :---------------------------------: | :----------------------------: |
|     策略类型     | off-policy 异策略，确定性策略 | off-policy 异策略，确定性策略 |     off-policy 异策略，随机策略     |   on-policy 同策略，随机策略   |
|   核心优化目标   |    最大化累计奖励，无熵项     |    最大化累计奖励，无熵项     | 最大化累计奖励+策略熵（双核心目标） | 最大化累计奖励，熵为辅助正则项 |
|   价值网络设计   |   双独立 Q 网络，抑制过估计   |      单 Q 网络，易过估计      |        双 Q 网络，抑制过估计        |    单 V 网络，估计优势函数     |
| 原生适配动作空间 |         连续动作空间          |         连续动作空间          |            连续动作空间             |          离散动作空间          |
|    训练稳定性    |     极强，几乎无崩溃风险      |       极差，易发散崩溃        |                极强                 |     中等，受梯度方差影响大     |
|     样本效率     |              高               |              高               |                极高                 |               低               |
|     调参难度     |             中等              |             极高              |           低（自适应 α）            |              中等              |
|   探索实现方式   |     外加高斯噪声补充探索      |     外加高斯噪声补充探索      |      最大熵随机策略，天然探索       |    随机策略+熵正则辅助探索     |
|   部署推理延迟   |      极低（确定性策略）       |      极低（确定性策略）       |         中等（需动作采样）          |       中等（需动作采样）       |
|  环境抗扰鲁棒性  |      良好，抗扰动能力强       |      差，对噪声高度敏感       |                极强                 |              中等              |

**在此项目中这四个算法在训练后的成功率都可以达到 90% 以上，且训练曲线相对平稳，未出现明显的崩溃发散现象。** 

## 项目概览

- **目的**：使用强化学习训练 FR3 机械臂完成目标到达任务（reach）。
- **算法**：包含 PPO、A2C、SAC、DDPG、TD3 等常见策略优化算法的训练与测试脚本。
...
- `FR3_PPO/`、`FR3_A2C/`、`FR3_SAC/`、`FR3_TD3/`、`FR3_DDPG/`：各算法的配置、环境封装、训练与测试脚本。
- `utils/`：环境封装（如 `fr3_env.py`）和其他工具函数。
